...
def enjoy(cfg: Config) -> Tuple[StatusCode, float]:
    ...
    # If you don't want to run load_state_dict with strict=False, then you have to load in those extra keys from this model
    checkpoints2 = [
        '../train_dir/DoomBasicCPU/checkpoint_p0/checkpoint_000000230_942080.pth',
        '../train_dir/DoomBasicCPU/checkpoint_p0/checkpoint_000000247_1011712.pth',
    ]

    # doom_deathmatch_bots PPO
    checkpoint_dict = Learner.load_checkpoint(checkpoints, device)
    # Reshape by padding with random numbers... works well enough (?)
    checkpoint_dict['model']['core.core.weight_ih_l0'] = torch.cat([checkpoint_dict['model']['core.core.weight_ih_l0'], torch.randn(1536, 128).to("cpu" if cfg.device == "cpu" else "cuda")], dim=1)
    actor_critic.load_state_dict(checkpoint_dict['model'], False)  # Strict mode off
    print(checkpoint_dict.keys())
    # print(checkpoint_dict['optimizer'])
    # print(checkpoint_dict['model'])

    # arnold health supreme loads but doesn't work properly, strange cause the controls seem to be identical to the WAD the arnold repo has
    # checkpoint_dict = Learner.load_checkpoint(['../train_dir/arnold/checkpoint_p0/checkpoint_000000002_8192.pth'], device)
    # actor_critic.load_state_dict(checkpoint_dict, False)

    # actor_critic.load_state_dict(checkpoint_dict)
