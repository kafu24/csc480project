...
def register_model_args(parser, args):
    ...
    """
    # network type
    parser.add_argument("--network_type", type=str, default='dqn_rnn',
                        help="Network type (DQN)")
    ...
    params, _ = parser.parse_known_args(args)
    print(params.bucket_size)
    assert params.clip_delta >= 0
    assert len(params.bucket_size) >= 1
